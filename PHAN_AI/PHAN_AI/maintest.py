...
from ultralytics import YOLO # YOLO - mô hình AI nhận diện
import aiohttp # HTTP client bất đồng bộ
import asyncio  #Lập trình bất đồng bộ
...
class Config:
    MODEL_PATH = r"D:\DO AN TOT NGHIEP\PHAN_AI\weights\best.pt"
    TELEGRAM_BOT_TOKEN = "7854048371:AAHLrpp3ZxSeiNAGS49n8-W_Nx3iUeqFbT4"
    TELEGRAM_CHAT_ID = "-1002524466925"
    # Sử dụng proxy Cloudflare Worker
    TELEGRAM_PROXY_URL = "https://apitele.hoan4408.workers.dev/"
    IMAGE_FOLDER = r"D:\DO AN TOT NGHIEP\PHAN_AI\testanh"
    OUTPUT_DIR = "unhealthy_crops"
    TEMP_IMAGE = "temp_camera.jpg"
    CONFIDENCE_THRESHOLD = 0.3 #ngưỡng tin cậy detection
    IOU_THRESHOLD = 0.6 # ngưỡng Non-Maximum
    MAX_DETECTIONS = 100
    CANVAS_SIZE = (500, 500)
...
class DetectionResult: #Kết quả nhận diện
    def __init__(self, label: str, confidence: float, bbox: tuple, image_path: str = None):
        self.label = label #labels 
        self.confidence = confidence # độ tin cậy 
        self.bbox = bbox  #toạ độ bounding box
        self.image_path = image_path # Đường dẫn ảnh gốc

class ModelManager:
    def __init__(self, model_path: str):
        self.model = YOLO(model_path) # Tải mô hình YOLO từ file .pt

    def predict(self, image: np.ndarray, conf: float = 0.3, iou: float = 0.6, max_det: int = 100) -> list: #Chạy inference trên ảnh với các tham số
...
class TelegramManager:
    def __init__(self, token: str, chat_id: str, proxy_url: str):
        ...
        self.proxy_url = proxy_url.rstrip('/')  # Loại bỏ dấu / cuối nếu có
    ...
    async def send_notification(self, image: np.ndarray, message: str) -> bool:
        try:
            ...
            # Tạo URL API thông qua proxy
            api_url = f"{self.proxy_url}/bot{self.token}/sendPhoto"
            ...
            data.add_field('photo', BytesIO(buffer), filename='detection.jpg', content_type='image/jpeg')
            ...
            print("❌ Timeout khi gửi Telegram")
            return False
    ...
    async def test_connection(self) -> bool:
        """Test kết nối với Telegram API thông qua proxy"""
        try:
            api_url = f"{self.proxy_url}/bot{self.token}/getMe"
                ...
                async with session.get(api_url) as response:
                    if response.status == 200:
                        ...
                        if result.get('ok'):
                            ...
                            print(f"✅ Kết nối Telegram thành công! Bot: {bot_info.get('first_name', 'Unknown')}")
                            return True
                        else:
                            print(f"❌ Telegram API error: {result.get('description', 'Unknown error')}")
                            return False
            ...
            print(f"❌ Lỗi test kết nối Telegram: {e}")
            return False
    ...
    async def process_single_image(self, image_path: str, send_telegram: bool = False) -> tuple:
        ...
        if send_telegram:
            ...
            if unhealthy_detections:
                ...
                print("🚀 Đang gửi thông báo qua Telegram...")
...
class PlantDiseaseGUI:
    def __init__(self, root: tk.Tk, config: Config):
        ...
        self.telegram_manager = TelegramManager(config.TELEGRAM_BOT_TOKEN, config.TELEGRAM_CHAT_ID, config.TELEGRAM_PROXY_URL)
    ...
    def _setup_ui(self) -> None:
        self.root.title("Nhận diện sâu bệnh với YOLOv11 - Telegram Proxy")
        ...
        self.status_label = ttk.Label(canvas_frame, text="Sẵn sàng - Sử dụng Telegram Proxy", font=("Arial", 10))
        ...
        buttons = [
            ...
            ("Test Telegram", self._test_telegram)
        ]
    ...
    def _test_telegram(self) -> None:
        """Test kết nối Telegram thủ công"""
        def test_async():
            result = asyncio.run(self.telegram_manager.test_connection())
            if result:
                self._add_result_text("✅ Test Telegram thành công!")
            else:
                self._add_result_text("❌ Test Telegram thất bại!")
    ...
    def _run_detection(self) -> None:
        ...
        if os.path.isfile(self.image_path):
            ...
            if unhealthy_count > 0:
                ...
                self._add_result_text("🚀 Đang gửi thông báo qua Telegram...")
